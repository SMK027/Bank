...
/** @var array $currentHabilitations */
/** @var bool $isEditMode */

$isEdit = (bool) ($isEditMode ?? false);
...
<div class="page-header">
    <div>
        <h1>
            ...
            <?= $isEdit ? 'Habilitations du superviseur' : 'Nouveau superviseur' ?>
        </h1>
    </div>
...
<div class="card" style="max-width:520px;">
    <div class="card-body">
        <form method="POST"
              action="<?= $isEdit ? '/moderation/supervisors/' . (int) $supervisor['id'] . '/habilitations' : '/moderation/supervisors' ?>"
              autocomplete="off">
            <?= csrf_field() ?>
            ...
            <?php if ($isEdit): ?>
                <div style="margin-bottom:1rem;padding:0.85rem 1rem;border:1px solid var(--border-color);border-radius:12px;background:var(--surface-2);">
                    ...
                    <small class="text-muted"><code><?= e($supervisor['supervisor_id']) ?></code></small>
                </div>
            <?php endif; ?>
            ...
            <?php if (!$isEdit): ?>
                ...
                <div style="margin-bottom:0.75rem;">
                    ...
                    <small class="text-muted">
                        Lettres, chiffres, tiret, underscore (3–64 car.).
                    </small>
                </div>

                <div style="margin-bottom:1.25rem;">
                    <label class="form-label" for="pin">Code PIN</label>
            ...
            <div style="margin-bottom:1.25rem;">
                ...
                <div style="display:grid;gap:0.5rem;">
                    <?php foreach ($assignableHabilitations as $featureKey => $label): ?>
            ...
            <?php else: ?>
            <div style="margin-bottom:1.25rem;">
                <label class="form-label">Habilitations actives</label>
                <div style="display:grid;gap:0.5rem;">
                    <?php foreach ($assignableHabilitations as $featureKey => $label): ?>
                        <label style="display:flex;align-items:flex-start;gap:0.65rem;padding:0.75rem 0.85rem;border:1px solid var(--border-color);border-radius:12px;background:var(--surface-2);">
                            <input type="checkbox" name="habilitations[]" value="<?= e($featureKey) ?>" style="margin-top:0.2rem;" <?= in_array($featureKey, $currentHabilitations, true) ? 'checked' : '' ?>>
                            <span>
                                <strong><?= e($label) ?></strong><br>
                                <small class="text-muted"><?= e($featureKey) ?></small>
                            </span>
                ...
                <small class="text-muted" style="display:block;margin-top:0.5rem;">Décoche une fonctionnalité pour révoquer son bypass, coche-la pour l'autoriser à nouveau.</small>
            </div>
            <?php endif; ?>

            <button type="submit" class="btn btn-primary">
                <i class="bi bi-check-lg"></i>
                <?= $isEdit ? 'Enregistrer les habilitations' : 'Créer le superviseur' ?>
            </button>
        </form>
    </div>
</div>
