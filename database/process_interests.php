...
$currentRate = $rateModel->getCurrentRate();
if ($currentRate === null) {
    echo sprintf("[%s] Aucun taux d'intérêt configuré. Arrêt.\n", date('Y-m-d H:i:s'));
    exit(0);
}

echo sprintf("[%s] Calcul des intérêts %d — taux : %s %%\n",
    date('Y-m-d H:i:s'),
    $year,
    number_format($currentRate * 100, 2, ',', ' ')
);

$savings   = $accountModel->findBy(['type' => 'savings']);
$processed = 0;
$skipped   = 0;
$errors    = 0;
...
foreach ($savings as $account) {
    ...
    try {
        ...
        $cap = Account::typeHasCap('savings') && ($account['cap'] ?? 0) > 0
            ? (float) $account['cap']
            : null;
